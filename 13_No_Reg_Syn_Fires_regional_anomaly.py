...
def nofire_regfire_synfire_standardized_anomaly(reg, varname):
    ...
    #-----------------------------------------------------
    # load region mask and reproject to the CERRA projection
    ...
    region_mask = region_mask.rio.reproject(lcc_crs)
    ...
    #clip to region
    region_mask = region_mask.interp(x = var.x, y = var.y, method = "nearest") #expand region_mask to the CERRA domain
    ...
    #save
    var_nofire_mean.to_netcdf(f'/net/rain/hyclimm/data/projects/SynFire/WP1/No_vs_Reg_vs_Syn_fires_monthly_standardized_anomaly/{reg}_{varname}_mean_anomaly_nofire.nc')
    var_regfire_mean.to_netcdf(f'/net/rain/hyclimm/data/projects/SynFire/WP1/No_vs_Reg_vs_Syn_fires_monthly_standardized_anomaly/{reg}_{varname}_mean_anomaly_regfire.nc')
    var_synfire_mean.to_netcdf(f'/net/rain/hyclimm/data/projects/SynFire/WP1/No_vs_Reg_vs_Syn_fires_monthly_standardized_anomaly/{reg}_{varname}_mean_anomaly_synfire.nc')


#-----------------------------------------------------
with mp.Pool(20) as pool:
    pool.starmap(nofire_regfire_synfire_standardized_anomaly, 
                 [(reg, varname) for reg in ["BI", "SC", "ME", "NEA", "FR", "AL", "SEA", "IP", "WMD", "EMD"] for varname in ['mx2t', '2r', '10si', 'tp', '2t', 'fwi']])
